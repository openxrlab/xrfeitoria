# Ref:
# https://github.com/EpicGames/BlenderTools/blob/main/send2ue/dependencies/rpc/base_server.py
...
class BaseRPCServer:
    def __init__(self, name, port, is_thread=False):
        ...
        # logger.debug(f'Started RPC server "{name}"')
...
class BaseRPCServerManager:
    ...
    def start_server_thread(self):
        ...
        logger.debug(f'Started {self.name} at "{self.port}" in a thread...')
    ...
    def start_server_blocking(self):
        ...
        logger.info(f'Started {self.name} at "{self.port}" in a blocking way...')
    ...
    def start(self, threaded=True):
        ...
        # start the blocking server
            ...
            logger.debug(f'RPC server "{self.name}" at {self.port} is already running...')

    def shutdown(self):
        """Shuts down the server."""
        if self.server_thread:
            logger.debug(f'RPC server "{self.name}" is shutting down...')
            ...
            logger.debug(f'RPC server "{self.name}" has shutdown.')
