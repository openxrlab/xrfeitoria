...
class Logger:
    ...
    @classmethod
    def setup_logging(
        cls, level: str = "INFO", log_path: "Optional[Path]" = None, replace: bool = True
    ) -> "loguru.Logger":
        ...
        logger.add(RichHandler(rich_tracebacks=True), level=level, format=cls.logger_format)
        if log_path:
            # add file logger
            ...
            logger.add(log_path, filter=cls.filter_unique, format=cls.logger_format)
            logger.info(f"Python Logging to {log_path}")
        return logger
